...
constructor(
    ...
    fun `test get space and not found`() {
        webTestClient.get().uri("/spaces/-1")
            .header("Authorization", "Bearer $creatorToken")
            .exchange()
            .expectStatus().isNotFound // Check status
    ...
    fun createSpace(
        ...
        val responseDTO = webTestClient.post().uri("/spaces")
            .header("Authorization", "Bearer $creatorToken")
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(requestDTO) // Use the request DTO
            .exchange()
            .expectStatus().isOk // Expect HTTP 200 OK (as per original test)
        ...
        classificationTopics.forEach { topicId ->
            assertTrue(returnedTopicIds.contains(topicId), "Expected topic ID $topicId not found in response")
        }
    ...
    fun `test create multiple spaces`() { // Renamed for clarity
        ...
        spaceIdOfBeforeLast = createSpace(
            creatorToken,
    ...
    fun `test get space by id`() { // Renamed
        webTestClient.get().uri("/spaces/$spaceId")
            .header("Authorization", "Bearer $creatorToken")
            .exchange()
            .expectStatus().isOk
    ...
    fun `test create space with existing name fails`() { // Renamed
        ...
        webTestClient.post().uri("/spaces")
            .header("Authorization", "Bearer $creatorToken")
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(requestDTO)
            .exchange()
            .expectStatus().isEqualTo(HttpStatus.CONFLICT) // Use HttpStatus enum
    ...
    fun `test patch space with empty request`() { // Renamed
        ...
        webTestClient.patch().uri("/spaces/$spaceId")
            .header("Authorization", "Bearer $creatorToken")
            .contentType(MediaType.APPLICATION_JSON)
            ...
            .value { response ->
                ...
                assertNotNull(ownerAdmin)
                assertEquals(SpaceAdminRoleTypeDTO.OWNER, ownerAdmin!!.role)
                // Note: enableRank was false before, check if it stays false (or true if changed in test 60)
    ...
    fun `test patch space with full request`() { // Renamed
        ...
        val requestDTO = PatchSpaceRequestDTO(
        ...
        webTestClient.patch().uri("/spaces/$spaceId")
            ...
            .value { response ->
                ...
                updatedTopics.forEach { topicId ->
                    assertTrue(returnedTopicIds.contains(topicId), "Expected updated topic ID $topicId not found")
                }
                assertFalse(returnedTopicIds.contains(topics[0]), "Topic ID ${topics[0]} should have been removed")
            }
    ...
    fun `test create space categories`() { // Renamed
        ...
        val initialSpace = webTestClient.get().uri("/spaces/$spaceId")
            .header("Authorization", "Bearer $creatorToken")
            .exchange()
            .expectStatus().isOk
            .expectBody<GetSpace200ResponseDTO>()
            .returnResult()
            .responseBody?.data?.space
        ...
        val createCategory1DTO = CreateSpaceCategoryRequestDTO(
            name = category1Name,
            description = category1Desc,
            displayOrder = category1Order
        )

        val createdCategory1 = webTestClient.post().uri("/spaces/$spaceId/categories")
            .header("Authorization", "Bearer $creatorToken")
            .contentType(MediaType.APPLICATION_JSON)
        ...
        val createCategory2DTO = CreateSpaceCategoryRequestDTO(
            name = category2Name,
            description = category2Desc,
            displayOrder = category2Order
        )

        val createdCategory2 = webTestClient.post().uri("/spaces/$spaceId/categories")
            .header("Authorization", "Bearer $creatorToken")
            .contentType(MediaType.APPLICATION_JSON)
    ...
    fun `test list space categories`() { // Renamed
        webTestClient.get().uri("/spaces/$spaceId/categories")
            ...
            .value { response ->
                ...
                assertTrue(categoryMap.containsKey(defaultCategoryId), "Default category should be listed")
                assertTrue(categoryMap.containsKey(categoryIds[0]), "First created category should be listed")
                assertTrue(categoryMap.containsKey(categoryIds[1]), "Second created category should be listed")
    ...
    fun `test update space category`() { // Renamed
        ...
        val requestDTO = UpdateSpaceCategoryRequestDTO(
            name = updatedName,
            description = updatedDesc,
            displayOrder = updatedOrder
        )

        webTestClient.patch().uri("/spaces/$spaceId/categories/$categoryToUpdateId")
            .header("Authorization", "Bearer $creatorToken")
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(requestDTO)
            .exchange()
    ...
    fun `test archive and unarchive category`() { // Renamed
        ...
        webTestClient.post().uri("/spaces/$spaceId/categories/$categoryToArchiveId/archive")
            .header("Authorization", "Bearer $creatorToken")
            .exchange()
            .expectStatus().isOk
            ...
            .value { response ->
                ...
                assertNotNull(response.data!!.category.archivedAt, "archivedAt should not be null after archiving")
            }
        ...
        webTestClient.get().uri("/spaces/$spaceId/categories")
            ...
            .value { response ->
                ...
                val categories = response.data!!.categories
                assertEquals(2, categories.size, "Only 2 categories should be listed by default now")
                assertNull(categories.find { it.id == categoryToArchiveId }, "Archived category should not be present")
            }

        // 3. Verify it IS listed when including archived
        webTestClient.get().uri { builder ->
            builder.path("/spaces/$spaceId/categories")
            ...
            .value { response ->
                assertNotNull(response.data?.categories)
                val categories = response.data!!.categories
                assertEquals(3, categories.size, "All 3 categories should be listed when including archived")
                assertNotNull(categories.find { it.id == categoryToArchiveId }, "Archived category should be present")
            }

        // 4. Unarchive the category
        // Assuming the unarchive endpoint uses DELETE and returns the updated category DTO
        webTestClient.delete().uri("/spaces/$spaceId/categories/$categoryToArchiveId/archive")
            .header("Authorization", "Bearer $creatorToken")
            .exchange()
            .expectStatus().isOk
            // Assuming it returns CreateSpaceCategory201ResponseDTO or similar
            ...
            .value { response ->
                assertNotNull(response.data?.category)
                assertEquals(categoryToArchiveId, response.data!!.category.id)
                assertNull(response.data!!.category.archivedAt, "archivedAt should be null after unarchiving")
            }
        ...
        webTestClient.get().uri("/spaces/$spaceId/categories")
            ...
            .value { response ->
                ...
                assertNotNull(categories.find { it.id == categoryToArchiveId }, "Unarchived category should be present")
            }
    }
    ...
    fun `test delete category fails for current default`() { // Renamed
        ...
        webTestClient.delete().uri("/spaces/$spaceId/categories/$currentDefaultCategoryId")
            .header("Authorization", "Bearer $creatorToken")
            .exchange()
    ...
    fun `test delete non-default category`() { // Renamed
        ...
        webTestClient.patch().uri("/spaces/$spaceId")
            ...
            .bodyValue(requestDTO)
            .exchange()
            .expectStatus().isOk // Ensure default is changed back

        // 2. Now delete the category that WAS default (categoryIds[0])
        val categoryToDeleteId = categoryIds[0]
        webTestClient.delete().uri("/spaces/$spaceId/categories/$categoryToDeleteId")
            .header("Authorization", "Bearer $creatorToken")
            .exchange()
        ...
        webTestClient.get().uri("/spaces/$spaceId/categories")
            ...
            .value { response ->
                ...
                assertNull(categories.find { it.id == categoryToDeleteId }, "Deleted category should not be present")
                assertNotNull(
                    categories.find { it.id == defaultCategoryId },
                    "Original default category should be present"
                )
                assertNotNull(categories.find { it.id == categoryIds[1] }, "Other category should be present")
            }
    }
    ...
    fun `test enumerate spaces default pagination and sorting`() { // Renamed
        webTestClient.get().uri { builder -> builder.path("/spaces").queryParam("page_size", 5).build() }
            .header("Authorization", "Bearer $creatorToken")
            .exchange()
            .expectStatus().isOk
            ...
            .value { response ->
                ...
                // nextStart should be the ID of the *last* item in the current page if using cursor pagination based on ID
                // assertEquals(spaceId, page.nextStart) // Verify this based on your pagination implementation
            }
    }
...
//                assertEquals(spaceIdOfSecond, spaces[3].id) // This one was created just after spaceId
                assertEquals("$originalSpaceName 02", spaces[3].name)

                assertEquals(spaceId, page!!.pageStart, "pageStart should be the ID of the first item")
    ...
    fun `test enumerate spaces sort by updatedAt desc with start`() { // Renamed
        webTestClient.get().uri { builder ->
            builder.path("/spaces")
                .queryParam("page_start", spaceIdOfLast) // Start after this ID (exclusive) if cursor based
                .queryParam("sort_by", "updatedAt")
            ...
            .value { response ->
                ...
                // Assuming cursor based on ID: We start *after* spaceIdOfLast. The next most recently updated is spaceIdOfBeforeLast
                // Let's re-evaluate the previous test's order: Updated(spaceId), Created(Last), Created(BeforeLast), Created(Second), ...
...
//                assertEquals(spaceIdOfLast, spaces[0].id)
                ...
                assertTrue(page.hasMore)
                assertNotNull(page.nextStart)
                // The next one after spaceIdOfLast in the updatedAt desc order is spaceIdOfBeforeLast
    ...
    fun `test add space admin`() { // Renamed
        ...
        val requestDTO = PostSpaceAdminRequestDTO(
            role = SpaceAdminRoleTypeDTO.ADMIN, // Use enum DTO
            userId = admin.userId
        )

        webTestClient.post().uri("/spaces/$spaceId/managers") // Assuming '/managers' is the correct path
            .header("Authorization", "Bearer $creatorToken")
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(requestDTO)
            .exchange()
            ...
            .value { response ->
                ...
                assertTrue(
                    admins.any { it.user.id == creator.userId && it.role == SpaceAdminRoleTypeDTO.OWNER },
                    "Original owner missing or role changed"
                )
                assertTrue(
                    admins.any { it.user.id == admin.userId && it.role == SpaceAdminRoleTypeDTO.ADMIN },
    ...
    fun `test add space admin and ship ownership`() { // Renamed
        ...
        webTestClient.post().uri("/spaces/$spaceId/managers")
            ...
            .value { response ->
                ...
                assertTrue(
                    admins.any { it.user.id == newOwner.userId && it.role == SpaceAdminRoleTypeDTO.OWNER },
                    "New owner not found or incorrect role"
                )
                // Verify old owner is now ADMIN
                assertTrue(
                    admins.any { it.user.id == creator.userId && it.role == SpaceAdminRoleTypeDTO.ADMIN },
    ...
    fun `test ship space ownership via patch`() { // Renamed
        ...
        webTestClient.patch().uri("/spaces/$spaceId/managers/${creator.userId}") // Target the user to change role
            .header("Authorization", "Bearer $newOwnerToken") // Action performed by the current owner
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(requestDTO)
            .exchange()
            .expectStatus().isOk
            ...
            .value { response ->
                ...
                assertTrue(
                    admins.any { it.user.id == newOwner.userId && it.role == SpaceAdminRoleTypeDTO.ADMIN },
    ...
    fun `test remove admin`() { // Renamed
        ...
        webTestClient.delete().uri("/spaces/$spaceId/managers/${newOwner.userId}")
            .header("Authorization", "Bearer $creatorToken") // Creator is owner again after test 110
            .exchange()
            .expectStatus().isOk // Assuming OK is returned on successful deletion
        ...
        webTestClient.get().uri("/spaces/$spaceId")
            ...
            .value { response ->
                assertNotNull(response.data.space.admins)
                val admins = response.data.space.admins
                assertNull(admins.find { it.user.id == newOwner.userId }, "Removed admin should not be present")
                assertNotNull(
                    admins.find { it.user.id == creator.userId && it.role == SpaceAdminRoleTypeDTO.OWNER },
                    "Owner should still be present"
                )
                assertNotNull(
                    admins.find { it.user.id == admin.userId && it.role == SpaceAdminRoleTypeDTO.ADMIN },
    ...
    fun `test delete space as admin`() { // Renamed
        ...
        // Test 110 made creator owner again. Test 120 removed newOwner. state: creator(owner), admin(admin).

        webTestClient.delete().uri("/spaces/$spaceId")
            ...
            .expectStatus().isOk // Assuming 200 OK based on original DTO name DeleteSpace200ResponseDTO
        ...
        webTestClient.get().uri("/spaces/$spaceId")
            .header("Authorization", "Bearer $creatorToken")
            .exchange()
            .expectStatus().isNotFound // Should now be Not Found
    }
}