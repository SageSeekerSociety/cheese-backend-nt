...
constructor(
    ...
    fun joinTask(taskId: IdType, participantId: IdType, participantToken: String): IdType {
        ...
        val response = webTestClient.post()
            .uri { builder -> builder.path("/tasks/$taskId/participants").queryParam("member", participantId).build() }
    ...
    fun submitTask(
        ...
        val response = webTestClient.post().uri("/tasks/$taskId/participants/$participantMembershipId/submissions")
            .header("Authorization", "Bearer $participantToken")
            .contentType(MediaType.APPLICATION_JSON)
    ...
    fun prepare() {
        ...
        approveTaskParticipant(creatorToken, taskId, participantTaskMembershipId) // Approve participant
        submissionId = submitTask(taskId, participantTaskMembershipId, participantToken, attachmentId) // Submit
    }
    ...
    fun `test get submissions - queryReview=true - when not reviewed`() {
        webTestClient.get()
            .uri { builder ->
                builder.path("/tasks/$taskId/participants/$participantTaskMembershipId/submissions")
                    .queryParam("queryReview", "true").build()
            }
            .header("Authorization", "Bearer $participantToken") // Participant gets their submission
            .exchange()
            .expectStatus().isOk
    ...
    fun `test get submissions - queryReview=true, reviewed=true - when not reviewed`() {
        webTestClient.get()
            ...
            .value { response ->
                assertNotNull(response.data.submissions)
                assertTrue(response.data.submissions.isEmpty(), "Should find no reviewed submissions yet")
            }
    }
    ...
    fun `test create review - success`() {
        ...
        val requestDTO = PostTaskSubmissionReviewRequestDTO(accepted = true, score = 5, comment = "Good job!")

        webTestClient.post()
            .uri("/tasks/$taskId/participants/$participantTaskMembershipId/submissions/$submissionId/review")
            .header("Authorization", "Bearer $creatorToken") // Creator reviews
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(requestDTO)
            .exchange()
            .expectStatus().isOk
            .expectBody<PostTaskSubmissionReview200ResponseDTO>() // Expect the specific DTO for review creation
            .value { response ->
    ...
    fun `test update review - success`() {
        ...
        val requestDTO = PatchTaskSubmissionReviewRequestDTO(accepted = false, score = 4, comment = "Could be better.")
    ...
    fun `test get submissions - queryReview=true, reviewed=false - after update`() { // Renamed
        webTestClient.get()
            ...
            .value { response ->
                assertNotNull(response.data.submissions)
                assertTrue(response.data.submissions.isEmpty(), "Should find no unreviewed submissions now")
            }
    }
    ...
    fun `test delete review - success`() {
        webTestClient.delete()
            ...
            .exchange()
            // Check status - Assuming 200 OK based on original test (though 204 No Content is common)
            .expectStatus().isOk
    ...
    fun `test get submissions - queryReview=true, reviewed=false - after recreation`() { // Renamed
        webTestClient.get()
            ...
            .value { response ->
                assertNotNull(response.data.submissions)
                assertTrue(response.data.submissions.isEmpty(), "Should be no unreviewed submissions")
            }
    }
}
