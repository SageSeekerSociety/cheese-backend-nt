...
class TaskAdminMBean(
    ...
    @ManagedOperation(description = "Create missing team member snapshots for older team participations (runs asynchronously).")
    ...
    fun createMissingTeamSnapshots(): String {
        ...
        if (isSnapshotTaskRunning.compareAndSet(false, true)) {
            try {
                ...
                CompletableFuture.runAsync({
                    try {
                        ...
                        val result = taskMembershipService.createMissingTeamSnapshotsForAllTasks()
                        logger.info("[JMX BG Task] Missing snapshot creation finished. Result: {}", result)
                    } catch (e: Exception) {
                        logger.error("[JMX BG Task] Error during background snapshot creation.", e)
                    } finally {
                ...
                val message = "Snapshot creation task started asynchronously. Check server logs for progress and results."
                logger.info("[JMX] {}", message)
            ...
            val message = "Snapshot creation task is already running. Please wait for it to complete."
            logger.warn("[JMX] {}", message)
            return "WARN: $message"
        }
    }

    @ManagedOperation(description = "Check if the missing snapshot creation task is currently running.")
    ...
    fun enableTaskRealNameRequirement(taskId: IdType): String {
        ...
        return try {
            ...
            logger.warn("[JMX] Precondition failed for enabling real name on task {}: {}", taskId, e.message)
    ...
    fun disableTaskRealNameRequirement(taskId: IdType): String {
        logger.info("[JMX] Operation triggered: Disable 'Require Real Name' for task ID: {}", taskId)
        return try {
            ...
            logger.warn("[JMX] Precondition failed for disabling real name on task {}: {}", taskId, e.message)
            ...
            logger.error("[JMX] Error disabling real name for task ID: {}: {}", taskId, e.message, e)
