    ...
    fun createMissingTeamSnapshotsForAllTasks(): SnapshotCreationResult {
        ...
        while (hasMore) {
            ...
            try {
                ...
                // Ideally, add a query to fetch only those where teamMembersRealNameInfo might be
                // empty,
                // but checking after fetch is reliable if a direct query is hard.
                membershipPage =
                    taskMembershipRepository.findAllByIsTeam(
                        true,
                        pageable,
                    ) // Assuming this query exists
            } catch (e: Exception) {
                logger.error(
            ...
            for (membership in membershipsInBatch) {
                ...
                if (membership.teamMembersRealNameInfo.isEmpty()) {
                    logger.debug(
                        "Membership ID {} (Team ID {}) needs snapshot.",
                        membership.id,
                        membership.memberId,
                    )
                    try {
                        ...
                        if (teamId == null) {
                            logger.warn(
                                "Membership ID {} has null memberId (Team ID). Skipping.",
                                membership.id,
                            )
                            continue
                        }
                        ...
                        if (currentTeamMembers.isNotEmpty()) {
                            val newSnapshot =
                                currentTeamMembers.map { teamMember ->
                                    // participantMemberUuid is generated by default value in
                                    // constructor now
                                    TeamMemberRealNameInfo(
                                        memberId = teamMember.user.id,
                                        realNameInfo =
                                            DefaultRealNameInfo, // Use default for non-real-name
                                        // tasks
                                    )
                                }
                            membership.teamMembersRealNameInfo.clear()
                            ...
                            logger.debug(
                                "Prepared snapshot with {} members for membership ID {}",
                                newSnapshot.size,
                                membership.id,
                            )
                        } else {
                            logger.warn(
                                "Team ID {} has no members currently. Creating empty snapshot for membership ID {}.",
                                teamId,
                                membership.id,
                            )
                            // Add empty list to mark it as processed (optional, depends on desired
                            // behavior)
                            membership.teamMembersRealNameInfo.clear()
                            membershipsToUpdateInBatch.add(
                                membership
                            ) // Save even with empty snapshot to mark as done
                        }
                    } catch (e: Exception) {
                        logger.error(
            ...
            if (membershipsToUpdateInBatch.isNotEmpty()) {
                try {
                    ...
                    logger.info(
                        "Saved batch of {} memberships with new/updated snapshots (Page {}).",
                        membershipsToUpdateInBatch.size,
                        pageNumber,
                    )
                } catch (e: Exception) {
                    logger.error("Failed to save batch for page {}: {}", pageNumber, e.message, e)
                    errorCount +=
                        membershipsToUpdateInBatch.size // Count all in failed batch as errors
