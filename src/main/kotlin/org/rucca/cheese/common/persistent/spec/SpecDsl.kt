...
/** Top-level DSL scope for configuring `where`, `having`, `order by`, `group by`, etc. */
@SpecDsl
class SpecContext<T : Any>(
    ...
    fun where(block: WhereContext<T>.() -> Unit) {
    ...
    fun having(block: WhereContext<T>.() -> Unit) {
        val context = WhereContext(query, cb, root, defaultIdProp)
        context.block()
    ...
    /**
     * Order results by ParadeDB full-text `pdb_score(id)` if the query is not a count query.
     *
     * @param idProperty entity id property; defaults to [useId] value
     * @param direction sort direction, default DESC
     */
    fun orderByParadeScore(
        idProperty: KProperty1<T, *>? = defaultIdProp,
        direction: Sort.Direction = Sort.Direction.DESC,
    ) {
        val id = requireNotNull(idProperty) { "orderByParadeScore requires an id property" }
        val isCount =
    ...
    /** Replace existing `ORDER BY` with provided orders. */
    fun orderBy(vararg orders: Order) {
        if (orders.isNotEmpty()) query.orderBy(*orders)
    }

    // ----- Order helpers -----
    fun <R> asc(ref: KProperty1<T, R>): Order = cb.asc(prop(ref).toPath(root))

    fun <R> desc(ref: KProperty1<T, R>): Order = cb.desc(prop(ref).toPath(root))

    fun <R> asc(ref: PathRef<T, R>): Order = cb.asc(ref.toPath(root))

    fun <R> desc(ref: PathRef<T, R>): Order = cb.desc(ref.toPath(root))

    fun ascIgnoreCase(ref: KProperty1<T, String?>): Order = cb.asc(cb.lower(prop(ref).toPath(root)))

    fun descIgnoreCase(ref: KProperty1<T, String?>): Order =
        cb.desc(cb.lower(prop(ref).toPath(root)))

    // Null-ordering helpers
    fun <R> ascNullsLast(ref: KProperty1<T, R>): Array<Order> {
    ...
    fun <R> descNullsLast(ref: KProperty1<T, R>): Array<Order> {
        val expr = prop(ref).toPath(root) as Expression<*>
        val nulls = cb.asc(cb.selectCase<Int>().`when`(cb.isNull(expr), 1).otherwise(0))
        return arrayOf(nulls, cb.desc(expr))
    }

    fun <R> ascNullsFirst(ref: KProperty1<T, R>): Array<Order> {
        val expr = prop(ref).toPath(root) as Expression<*>
        val nulls = cb.asc(cb.selectCase<Int>().`when`(cb.isNull(expr), 0).otherwise(1))
        return arrayOf(nulls, cb.asc(expr))
    }

    fun <R> descNullsFirst(ref: KProperty1<T, R>): Array<Order> {
        val expr = prop(ref).toPath(root) as Expression<*>
        val nulls = cb.asc(cb.selectCase<Int>().`when`(cb.isNull(expr), 0).otherwise(1))
        return arrayOf(nulls, cb.desc(expr))
    }
...
class ConditionalClause<E : Any>(
    ...
    infix fun orElse(block: WhereContext<E>.() -> Unit) {
        if (!conditionMet) {
            context.block()
        }
    }
...
open class WhereContext<E : Any>(
    ...
    private fun addParadePredicate(query: SearchQuery, idProperty: KProperty1<E, *>) {
        val jsonbExpr = cb.function("to_jsonb", Any::class.java, cb.literal(query.toJsonString()))
        val pred =
            cb.isTrue(
                cb.function(
                    "pg_search",
    ...
    fun parade(
        ...
        val queryObj = SearchQueryBuilder<E>().build(block = build)
    ...
    // ----- Subqueries -----
    inline fun <reified S : Any> exists(crossinline block: SubWhereContext<S, E>.() -> Unit) {
        ...
        val subWhereCtx = SubWhereContext(query, cb, subRoot, this.from, chain)
        subWhereCtx.block()
    ...
    inline fun <reified S : Any> notExists(crossinline block: SubWhereContext<S, E>.() -> Unit) {
        val sq = query.subquery(Int::class.java)
        val subRoot = sq.from(S::class.java)
...
class SubqueryContext<E : Any, P : Any, R : Any>(
    ...
    fun where(block: SubWhereContext<E, P>.() -> Unit) {
        val ctx = SubWhereContext<E, P>(mainQuery, cb, from, parentFrom, ancestors)
        ctx.block()
 ...
 */
@SpecDsl
class SubWhereContext<S : Any, P : Any>(
    query: CriteriaQuery<*>,
    cb: CriteriaBuilder,
    val fromS: From<*, S>,
    val parentFromP: From<*, P>,
    ...
    fun <X> parent(ref: KProperty1<P, X>): Path<X> = prop(ref).toPath(parentFromP)

    fun <X> parent(ref: PathRef<P, X>): Path<X> = ref.toPath(parentFromP)
